    ...
    try {
      ...
      const [eventsCount, schedulesCount, hubsCount] = await Promise.all([
        ...
        profile?.role === "business"
          ? supabase.from("hubs").select("id", { count: "exact" }).eq("owner_id", user.id).eq("is_active", true)
          : { count: 0 },
      ])
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-2">Here's what's happening in your action sports community</p>
        </div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{data.stats.totalSchedules}</div>
                </CardContent>
              </Card>
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{data.stats.totalHubs}</div>
                </CardContent>
              </Card>
            </div>
                        ...
                        <div key={event.id} className="flex items-center justify-between p-3 border rounded-lg">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {event.hubs.name} • {format(new Date(event.start_time), "MMM d, h:mm a")}
                            </p>
                          </div>
                ...
                <CardContent>
                  {data.mySchedules.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground mb-4">No training schedule set</p>
                      <Link href="/schedule">
                        <Button size="sm" className="bg-orange-600 hover:bg-orange-700">
                      ...
                      {data.mySchedules.map((schedule) => (
                        <div key={schedule.id} className="flex items-center justify-between p-3 border rounded-lg">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">{schedule.hubs.name}</p>
                          </div>
                        </div>
                      ))}
                    </div>
