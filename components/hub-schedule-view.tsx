...
export function HubScheduleView({ hubId }: HubScheduleViewProps) {
  ...
  const totalUsers = new Set(schedules.map((s) => `${s.users.first_name} ${s.users.last_name}`)).size
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground">
              {filterDay === "all" ? "No regular training schedules yet." : `No training scheduled for ${filterDay}.`}
            </p>
          </div>
        ) : filterDay === "all" ? (
                ...
                <div key={day}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {daySchedules.map((schedule) => (
                      <ScheduleCard key={schedule.id} schedule={schedule} showUser={true} />
                    ))}
                  </div>
                </div>
              )
